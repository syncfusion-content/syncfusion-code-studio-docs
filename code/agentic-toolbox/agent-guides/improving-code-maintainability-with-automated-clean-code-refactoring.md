...
- Use Syncfusion Code's [Inline Chat](/code-studio/agentic-toolbox/chat-interactions/inlinechat) to understand unfamiliar or complex code instantly
...
- Use [Agent mode](/code-studio/agentic-toolbox/chat-interactions/agent) to perform multi-file, large-scale refactoring automatically
...
- **[Ask mode](/code-studio/agentic-toolbox/chat-interactions/ask)**: For questions, explanations, and single-file changes
- **[Agent mode](/code-studio/agentic-toolbox/chat-interactions/agent)**: For complex, multi-file refactoring that requires the AI to autonomously plan and execute changes across multiple files

> **Tip:** Use [Inline Chat](/code-studio/agentic-toolbox/chat-interactions/inlinechat) for quick, focused changes to code you've selected. Use [Agent mode](/code-studio/agentic-toolbox/chat-interactions/agent) in the Chat Panel for larger refactoring tasks that involve multiple files.

## Steps to Refactor Code

### Step 1: Understand Code Before You Refactor

Before changing any code, you must understand what it does. Syncfusion Code's [Inline Chat](/code-studio/agentic-toolbox/chat-interactions/inlinechat) gives you an instant, plain-English explanation of any selected block of code â€” removing the guesswork before you begin.


### Step 2: Eliminate Repeated Code with Extracted Functions

**Code duplication** â€” the same logic copy-pasted in multiple places â€” is one of the most common maintainability problems. When the same operation appears more than once, every bug fix or change must be applied in every location, which is time-consuming and error-prone.
...
1. Select the entire file contents in the editor (`Ctrl+A` / `Cmd+A`).
   ...
   > **Tip:** When you select the entire file, Syncfusion Code's AI can analyze all the functions and automatically identify patterns where the same logic appears multiple times. You don't need to manually highlight each duplicate â€” the AI does this detection for you.
...
4. Syncfusion Code analyzes the entire file, identifies duplicate patterns, and suggests a refactored version:
   ...
   <img src="../agentic-toolbox-images/duplicate-code-after.png" alt="Inline Chat diff view showing duplicated logic extracted into a shared reusable function" />
...
Long-winded code with unnecessary variables, redundant checks, or outdated syntax is harder to scan and review. Syncfusion Code can rewrite unnecessarily verbose code into a compact, idiomatic form â€” without removing any functionality.
...
3. Type the following prompt and press Enter:
   ...
   ```
   <img src="../agentic-toolbox-images/verbose-code-after-functions.png" alt="Inline Chat diff view showing verbose variable declarations replaced with concise arrow functions" />

4. Syncfusion Code suggests a cleaner version:
   <img src="../agentic-toolbox-images/verbose-code-after-templates.png" alt="Inline Chat diff view showing string concatenation replaced with template literals" />
...
3. Type the following prompt and press Enter:
   ...
   ```
   <img src="../agentic-toolbox-images/large-function-split-emi.png" alt="Inline Chat diff view showing the god function split into a dedicated calculateEMI function" />

4. Syncfusion Code suggests the refactored version with each concern separated:

   <img src="../agentic-toolbox-images/large-function-split-complete.png" alt="Inline Chat diff view showing the remaining logic split into calculateTotals and formatCurrency functions" />
...
1. Place the cursor on the symbol you want to rename â€” this can be a variable, function name, class, or parameter.
2. Press `F2` to trigger the rename action.
3. Syncfusion Code displays a list of AI-suggested alternative names in a dropdown.

   <img src="../agentic-toolbox-images/symbol-rename-suggestions.png" alt="F2 rename dropdown showing AI-suggested alternative names for a poorly named variable" />
...
5. Press Enter to confirm. The symbol is automatically renamed **everywhere it is used** in the project â€” across all files.
...
> **Note:** The rename operation is scope-aware â€” it only affects the specific symbol you selected, not other variables that happen to have the same name elsewhere in your code.

### Step 6: Large-Scale Refactoring with Agent Mode

For bigger refactoring jobs that span multiple files â€” such as standardizing error handling patterns, migrating to a new API, or updating deprecated library methods across your entire codebase â€” **[Agent mode](/code-studio/agentic-toolbox/chat-interactions/agent)** is the right tool.

#### Example: Standardize Error Handling Across Multiple Files

Suppose your codebase has inconsistent error handling â€” some functions use `try-catch` blocks, others silently swallow errors, and error messages lack context. The goal is to apply a single, consistent error handling pattern across all service files.
...
3. Open the Chat Panel (if it's not already visible, press `Ctrl+Shift+P` (Windows/Linux) or `Cmd+Shift+P` (Mac) to open the **Command Palette** â€” a search box that appears at the top of the editor â€” then type "Chat: Focus on Chat View" and press Enter).
...
6. Watch [Agent mode](/code-studio/agentic-toolbox/chat-interactions/agent) work. It will automatically:
   ...
   - **Create automatic checkpoints** as it works â€” these are restore points you can return to if you want to undo all changes from the Agent session

7. Review each proposed change in the side-by-side diff view â€” click Keep to accept or Undo to reject individual file changes.
...
- [Quick Fix Error Guide](/code-studio/agentic-toolbox/chat-interactions/quick-fix-error) â€” Catch and fix bugs early with AI assistance.
- [Agent Mode](/code-studio/agentic-toolbox/chat-interactions/agent) â€” Explore advanced Agent workflows and capabilities.
