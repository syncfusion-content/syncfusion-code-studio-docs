...
description: Learn to use BoldCreate's AI to automatically refactor code for better maintainability without changing behavior.
...
BoldCreate makes this process faster and more consistent through its built-in AI-powered tools. In this tutorial, you'll walk through practical, real-world examples of how BoldCreate automatically detects and fixes the most common code quality problems: repeated logic, bloated functions, and poor naming – all with just a few keystrokes.
...
- Use BoldCreate's [Inline Chat](/code/agentic-toolbox/chat-interactions/inlinechat) to understand unfamiliar or complex code instantly
...
- Use [Agent mode](/code/agentic-toolbox/chat-interactions/agent) to perform multi-file, large-scale refactoring automatically


## Understanding BoldCreate's AI-Features

Before you begin refactoring, it's important to understand the two main AI-features you'll use:

### Inline Chat
**[Inline Chat](/code/agentic-toolbox/chat-interactions/inlinechat)** is a lightweight AI assistant that appears directly in your code editor as an input box overlay. You open it with `Ctrl+I` (Windows/Linux) or `Cmd+I` (Mac). Once open, you can type natural language instructions or use slash commands (like `/explain` or `/fix`) to modify the code you've selected. Inline Chat works on **a single file or selection** at a time and shows you a side-by-side diff preview before applying any changes.

### Chat Panel
The **Chat Panel** is a separate sidebar panel in BoldCreate where you can have ongoing conversations with the AI. You can use it to ask questions, get explanations, or give instructions. It supports two modes:
- **[Ask mode](/code/agentic-toolbox/chat-interactions/ask)**: For questions, explanations, and single-file changes
- **[Agent mode](/code/agentic-toolbox/chat-interactions/agent)**: For complex, multi-file refactoring that requires the AI to autonomously plan and execute changes across multiple files

> **Tip:** Use [Inline Chat](/code/agentic-toolbox/chat-interactions/inlinechat) for quick, focused changes to code you've selected. Use [Agent mode](/code/agentic-toolbox/chat-interactions/agent) in the Chat Panel for larger refactoring tasks that involve multiple files.

## Steps to Refactor Code

### Step 1: Understand Code Before You Refactor

Before changing any code, you must understand what it does. BoldCreate's [Inline Chat](/code/agentic-toolbox/chat-interactions/inlinechat) gives you an instant, plain-English explanation of any selected block of code - removing the guesswork before you begin.
...
BoldCreate can detect duplicated logic and extract it into a single, reusable function automatically.
...
1. Select the entire file contents in the editor (`Ctrl+A` / `Cmd+A`).
   ...
   > **Tip:** When you select the entire file, BoldCreate's AI can analyze all the functions and automatically identify patterns where the same logic appears multiple times. You don't need to manually highlight each duplicate – the AI does this detection for you.
...
3. Type the following prompt and press Enter:
   ...
   ```
4. BoldCreate analyzes the entire file, identifies duplicate patterns, and suggests a refactored version:
...
Long-winded code with unnecessary variables, redundant checks, or outdated syntax is harder to scan and review. BoldCreate can rewrite unnecessarily verbose code into a compact, idiomatic form – without removing any functionality.
...
4. BoldCreate suggests a cleaner version:
...
The solution is the **Single Responsibility Principle (SRP)**: each function should do exactly one thing. BoldCreate can split a god function into smaller, focused units automatically.
...
4. BoldCreate suggests the refactored version with each concern separated:
...
Poor variable and function names silently reduce code quality over time. Names like `d`, `tmp`, `fn`, or `process` tell the next developer nothing about what they represent. BoldCreate can suggest meaningful names and rename every reference across your entire project in a single action.
...
3. BoldCreate displays a list of AI-suggested alternative names in a dropdown.
...
For bigger refactoring jobs that span multiple files - such as standardizing error handling patterns, migrating to a new API, or updating deprecated library methods across your entire codebase - **[Agent mode](/code/agentic-toolbox/chat-interactions/agent)** is the right tool.
...
1. Clone the sample repository and open it in BoldCreate:
...
2. Open your project in BoldCreate.
...
4. Select **[Agent](/code/agentic-toolbox/chat-interactions/agent)** from the mode selector dropdown.
...
6. Watch [Agent mode](/code/agentic-toolbox/chat-interactions/agent) work. It will automatically:
...
- [Generate Your First Code Change Using Agent](/code/agentic-toolbox/agent-guides/generate-your-first-code-using-agent) – Let AI write new code for you.
- [Quick Fix Error Guide](/code/agentic-toolbox/chat-interactions/quick-fix-error) – Catch and fix bugs early with AI assistance.
- [Agent Mode](/code/agentic-toolbox/chat-interactions/agent) – Explore advanced Agent workflows and capabilities.
