...
description: Compare AI models in BoldCreate and learn when to use Claude, Gemini, and GPT families for coding, debugging, refactoring, and fast iterations with examples.
...
Each AI model available in BoldCreate has distinct strengths. To help you decide which model to use, this tutorial provides real-world examples of common developer tasks along with sample prompts, expected responses, and guidance on why a particular model is a good fit.
...
For a full list of supported models and side-by-side feature comparisons, see the [Model Comparison](/code/customize/model-comparison) page in the customization section.
...
- Identify the strengths and optimal use cases for each AI model in BoldCreate
...
Below are the AI models available in BoldCreate, explained through realistic tasks, example prompts, and expected results. Each section explains **why** a particular model fits a specific task type.
...
1. **Explore Advanced Features**: Try using the [Agent](/code/agentic-toolbox/chat-interactions/agent) feature with reasoning-focused models like GPT-5 or GPT-5.2 for autonomous multi-step workflows.
