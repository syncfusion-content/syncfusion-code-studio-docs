...
description: Learn how to use a custom Codebase Documenter agent in BoldCreate to generate clear, beginner-friendly documentation for any codebase.
platform: syncfusion-code-studio
keywords: documentation, custom-agent, codebase-docs, ai-documentation, boldcreate, knowledge-sharing
---
...
**.agent.md file**
A Markdown-based configuration file that defines a custom agent's identity, behavior, tools, and workflow instructions. BoldCreate reads these files from the `.boldcreate/agents/` folder and makes the agents they describe available in the Chat Panel.
...
1. Open your project folder in BoldCreate (`Ctrl+K Ctrl+O` (Windows/Linux) or `Cmd+K Cmd+O` (Mac)), or clone the repository and open the folder in BoldCreate. For this tutorial, we use the example project [ej2-showcase-react-loan-calculator](https://github.com/syncfusion/ej2-showcase-react-loan-calculator.git).

2. **Create the agent configuration folder (if it does not exist):**
   - In the **Explorer** (`Ctrl+Shift+E` (Windows/Linux) or `Cmd+Shift+E` (Mac)), create a folder named `.boldcreate` at the root of your project.
   - Inside it, create a subfolder named `agents`.
   - Final path: `.boldcreate/agents/`.
...
4. **Place the template in your project:**
   - Copy `Codebase-Documenter.agent.md` into the `.boldcreate/agents` folder.
5. **Open the agent file in the editor:**
   - In the **Explorer**, expand `.boldcreate/agents`.
...
<img src="./tutorials-images/fix-docs-agent-setup.png" alt="Explorer showing the .boldcreate/agents folder with the Codebase-Documenter.agent.md file" />
...
2. **Confirm the model and tools:**
   - Ensure the `model` is set to your preferred model in BoldCreate.
...
1. Open the **Chat Panel** in BoldCreate (`Ctrl+Alt+B` (Windows/Linux) or `Cmd+Alt+B` (Mac)).
...
> **Note:** If you do not see the agent in the dropdown, double-check that the `.agent.md` file is inside `.boldcreate/agents/`, the file name ends with `.agent.md`, and there are no syntax errors in the metadata block at the top. For a deeper understanding of agent behavior, see [Custom Agents](/code/customize/custom-agents).
...
1. **Choose your scope:**
   - If your project is already open in BoldCreate and you want docs for everything, you can target the **current workspace**.
