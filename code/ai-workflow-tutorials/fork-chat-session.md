---
title: Fork Chat Sessions to Explore Multiple Solutions in BoldCreate
...
keywords: boldcreate, IDE, AI, developer-tools, syncfusion, code-assistance, productivity, chat, fork, session, conversation, branch, history, checkpoint, alternative-approaches, parallel-development
---
...
When working on complex problems, you often need to explore multiple approaches simultaneously without losing your progress. Forking a chat session in BoldCreate allows you to create independent conversation branches from an existing session, each maintaining its own history and context.
...
BoldCreate provides two flexible ways to fork a chat session. Use Method 1 to fork the entire conversation, or use Method 2 to branch from a specific point in your chat history.
...
**Step 2**: BoldCreate creates a new chat session that includes all previous messages and context from the original session.
...
**Step 3**: BoldCreate creates a new session containing only the conversation history up to the selected checkpoint.
...
- [Checkpoints and Editing Requests](/code/agentic-toolbox/core-capabilities/checkpoints) - Restore your workspace to earlier states within a single session
- [Agent Mode](/code/agentic-toolbox/chat-interactions/agent) - Combine forked sessions with agent mode for autonomous exploration of different approaches
