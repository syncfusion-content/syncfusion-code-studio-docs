---
title: Hooks
description: Reference documentation for configuring hooks in BoldCreate to run custom shell commands at agent lifecycle events.
platform: syncfusion-code-studio
control: IDE
documentation: Getting Started
keywords: boldcreate, IDE, hooks, automation, agent session, shell commands, lifecycle, configuration
---
...
Hooks let you run custom shell commands at specific lifecycle points during an agent session in BoldCreate. Use hooks to automate repetitive tasks, enforce security policies, validate tool usage, and integrate external systems into AI-assisted workflows.
...
> **Important:** Hooks run locally with the same permissions as your BoldCreate session. Review every hook command and script carefully before enabling it.
...
1. BoldCreate with a workspace is open and you have write access to the workspace.
...
BoldCreate supports the following hook lifecycle events. Event names are case-sensitive and must use the exact PascalCase values shown below.
...
1. **Click** the Settings icon in the Chat window and **Select** **Agent Customizations**.
	...
	<img src="./customize-images/hookstart.png" alt="Chat settings menu in BoldCreate with the Settings icon highlighted" >
2. **Select** hooks and **click** **Generate Hooks**.

	<img src="./customize-images/hookgenerate.png" alt="Selecting hooks to generate hooks file" >

3. **Choose** the hook lifecycle event you want to configure for example we using SessionStart.

	<img src="./customize-images/hookselect.png" alt="Hook creation dialog showing hook event selection" >

4. **Enter** a descriptive hook name and press Enter.

	<img src="./customize-images/hookname.png" alt="Hook naming input field in BoldCreate" >

5. It will create a **Default Configuration** like below Image
	<img src="./customize-images/hookconfig.png" alt="Generated hook files created after naming a new hook" >

6. **Edit** the generated configuration.

	<img src="./customize-images/hookedit.png" alt="Generated hook configuration open for editing in the editor" >

7. **Run** a test prompt to verify the hook is active.
	<img src="./customize-images/hookusing.gif" alt="Animated demonstration of creating and configuring a hook in BoldCreate" >
...
|---|---|---|
| Hook does not run | Incorrect event name, file path, or configuration placement | Verify `.boldcreate/config.json`, event names, and referenced script paths |
...
- [Enhancing Security Reviews and Code Quality with Automated Hooks in BoldCreate](/code/agentic-toolbox/agent-guides/enhance-security-with-hooks)
