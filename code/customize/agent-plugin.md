...
description: Learn how to configure and control agent autonomy in BoldCreate using the new Autopilot and permissions picker. This tutorial walks you through enabling Autopilot with chat.autopilot.enabled, choosing between Default Approvals, Bypass Approvals, or Autopilot (Preview), and understanding the security implications of each mode
platform: syncfusion-code-studio
keywords: syncfusion, boldcreate, Autopilot, agent permissions, bypass approvals, agent-scoped hooks, agent debugging 
---
# Agent Plugins 

## Purpose 

Agent plugins are bundles of customizations you can install from plugin marketplaces. They extend BoldCreate with combinations of slash commands, skills, agents, hooks, and MCP servers, while working alongside your existing customizations. 
...
- Select the **settings** icon in the Extensions sidebar and choose **Agent Customizations**. 
    ...
    <img src="./customize-images/settings-page-plugin.png" alt="new_Custom_Agent" />

- Select Plugins from the left menu, then click Browse Marketplace to view the list of agent plugins available

    <img src="./customize-images/agent-plugin.png" alt="new_Custom_Agent" />

- Browse the available plugins from your configured marketplaces. 

    <img src="./customize-images/plugins-list.png" alt="Plugin List" />
...
- BoldCreate automatically downloads and registers the plugin. 

<img src="./customize-images/agent-plugin.gif" alt="Plugin List" />
...
- Run the command **Chat: Install Plugin From Source**. 
    ...
    <img src="./customize-images/git-plugin.png" alt="Plugin List" />

- Enter the Git repository UL (for example, `https://github.com/yourorg/custom-plugin`). 

    <img src="./customize-images/git-plugin-custom.png" alt="Plugin List" />

- BoldCreate automatically clones the repository and installs the plugin. 
...
2. From this view, you can perform the following actions: 
    ...
    <img src="./customize-images/agent-plugin-extension.png" alt="Plugin List" />

### Step 4: Update Plugins 

BoldCreate provides two methods to check for plugin updates: 

- **Automatic Updates**: BoldCreate checks for updates automatically every 24 hours if the `extensions.autoUpdate` setting is enabled. 
...
- Right-click the plugin you want to manage and select **Enable** or **Disable**. 
    ...
    <img src="./customize-images/agent-plugin-extension-disable.png" alt="Plugin List" />
...
Once you install a plugin, BoldCreate processes the plugin's configuration and makes its components available for use. Here is what happens behind the scenes: 
...
When you install a plugin, BoldCreate performs the following steps: 

1. **Configuration Reading**: BoldCreate reads the plugin's `plugin.json` file to understand the plugin's structure and components. 
...
- **Slash Commands**: New slash commands (for example, `/analyze` or `/format`) work immediately when you type them in chat. 
    ...
    <img src="./customize-images/agent-plugin-gif.gif" alt="Plugin List" />
...
- **[Agent Skills](/code/customize/skills)**: Learn how to create reusable, specialized tasks for agents. 

- **[Custom Agents](/code/customize/custom-agents)**: Understand how to build domain-specific AI personas. 

- **[MCP Servers](/code/customize/mcp/customservers)**: Discover how to integrate mcp tools and data sources. 
