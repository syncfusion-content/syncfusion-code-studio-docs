...
After upgrading to BoldCreate v2.0.0+, the method to review git diff changes has been updated. Previously(before v2.0.0), users could view uncommitted changes by typing **@Git Diff** in the chat box or using the **"Add context(+)"** button. In v2.0.0+, this functionality is now accessed by typing **#** in the chat input box and selecting `#changes` from the dropdown menu.
...
1. Make sure the desired project/workspace is open and active in BoldCreate — preferably one that is connected to a GitHub repository. The `#changes` tool only shows modifications from the currently opened project.

2. Go to the chat panel in BoldCreate where you interact with the AI assistant.
