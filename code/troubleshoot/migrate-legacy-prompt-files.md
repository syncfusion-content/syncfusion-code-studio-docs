...
## Problem Summary
Legacy BoldCreate prompt files (`.prompt` in `.boldcreate/prompts/`) from versions before 2.0.0 are not recognized or usable in BoldCreate 2.0.0 and later, which requires the new `.prompt.md` format with YAML frontmatter and Markdown support. Users may find their old prompts missing, not appearing in chat, or not functioning as expected.

## Possible Causes
- Prompt files are still in the legacy `.prompt` format (`.boldcreate/prompts/`)
- Files are not saved with the `.prompt.md` extension
- Prompts are not placed in the correct directory (`.boldcreate/prompts/` for workspace, user profile folder for user prompts)
...
- **Workspace prompts:** `.boldcreate/prompts/` (in your workspace folder)
- **File Extension:** `.prompt`

### 2. Open and Copy Prompt Content
1. Open a `.prompt` file in a text editor or BoldCreate.
2. Copy the content from the `prompt:` section (the main instruction text).

### 3. Create a New Prompt File in BoldCreate
You can create prompts as either **Workspace** or **User** prompts:
- **Workspace Prompt:** `.boldcreate/prompts/` 
...
> **Note:** For detailed instructions on creating and configuring prompt files, see the [Custom Prompt Files](/code/customize/custom-prompt) guide.
...
1. Open the new `.prompt.md` file in BoldCreate.
