...
description: "Learn how to build a Voice Notes web app using Spec-Driven Development (SDD) approach with BoldCreate"
...
# Build Your First App in BoldCreate 

## Overview

Ready to build your first app? In this guide, you'll create a **Voice Notes web app** directly in **BoldCreate** using a simple, step-by-step approach. BoldCreate guides you through planning, building, and organizing your project using clear commands that make the whole process stress-free.

You'll move from idea to working application by following **Spec-Driven Development (SDD)** — a beginner-friendly approach where you describe what you want to build, and BoldCreate generates the code for you. No complex setup. No guesswork. Just clear instructions and working code.
...
**Why it matters:**
- **Reduces confusion** — BoldCreate knows exactly what to build
...
OpenSpec is a tool built for Spec-Driven Development. It helps you organize your specifications, proposals, designs, and tasks all in one place within your project folder. When you use OpenSpec with BoldCreate:
...
3. **BoldCreate builds it** — The AI reads your organized specs and generates the code
...
For more information about OpenSpec, visit our [Using OpenSpec in BoldCreate](/code/integrations-and-apis/using-openspec-inside-syncfusion-code) guide.
...
Before building your Voice Notes app, ensure your BoldCreate environment is ready:


1. **Create a new workspace**:
   - Create a new folder named `voice-notes-app` on your machine
   - Open BoldCreate on your machine
   - Open that 'voice-notes-app'  as your workspace in BoldCreate by clicking **File** -> **Open Folder**
...
1. **Open the Terminal** in BoldCreate (`Terminal` → `New Terminal`)
...
3. **Initialize the spec structure** in your project folder:
   ...
   openspec init
   ```
   A prompt will appear asking you to select an AI tool. Choose **GitHub Copilot** from the options to connect OpenSpec with BoldCreate.
...
- **`.github/`** — Contains all the skills and prompts that OpenSpec uses to communicate with BoldCreate and generate your plans and implement it.
...
Now that your environment is set up, you'll create a detailed project plan. This plan helps BoldCreate understand exactly what you want to build before writing any code. You describe your app in simple language, and BoldCreate generates a structured plan that includes the proposal, design, specifications, and tasks.

To create your project plan, use the `/opsx-propose` command in the BoldCreate Chat Panel. Follow the command with a detailed description of what you want to build. BoldCreate will read your description and generate:
...
After you've reviewed and approved your project plan from Step 3, it's time to build the actual application. In this step, BoldCreate reads all your specifications and tasks, then automatically writes the code for your Voice Notes app. The AI works through each task one by one, creating files and writing code that matches your plan exactly.

To start the implementation, run the `/opsx-apply` command in the BoldCreate Chat Panel:
...
To finalize and organize your project, run the `/opsx-archive` command in the BoldCreate Chat Panel:
