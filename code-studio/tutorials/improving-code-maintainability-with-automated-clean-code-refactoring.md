...
- Use Code Studio's Inline Chat to understand unfamiliar or complex code instantly
...
- **Chat mode**: For questions, explanations, and single-file changes
- **Agent mode**: For complex, multi-file refactoring that requires the AI to autonomously plan and execute changes across multiple files

> **Quick tip:** Use Inline Chat for quick, focused changes to code you've selected. Use Agent mode in the Chat Panel for larger refactoring tasks that involve multiple files.
...
Before changing any code, you must understand what it does. Code Studio's Inline Chat gives you an instant, plain-English explanation of any selected block of code — removing the guesswork before you begin.
...
6. Code Studio returns a clear, step-by-step explanation of what the selected code does in the Inline Chat window.

   > **Visual Reference:** The images below show what the Inline Chat interface looks like when using the `/explain` command. If you're reading this in a code editor, these images will appear when the document is rendered in preview mode or published online.
...
<img src="./tutorials-images/duplicate-code-before.png" alt="TypeScript file showing duplicated calculation logic in multiple functions before refactoring" />
...
<img src="./tutorials-images/verbose-code-before.png" alt="TypeScript file with verbose code before refactoring — multiple unnecessary intermediate variables and string concatenation" />
...
<img src="./tutorials-images/large-function-before.png" alt="TypeScript file with a large calculateLoanDetails god function that handles EMI, totals, and formatting all in one place" />
...
For bigger refactoring jobs that span multiple files — such as standardizing error handling patterns, migrating to a new API, or updating deprecated library methods across your entire codebase — **Agent mode** is the right tool.

> **Key concept — Agent mode:** An AI mode in Code Studio that can autonomously plan, read, and edit multiple files in sequence to complete a multi-step task. Unlike Inline Chat (which works on a single file or selection), Agent mode can coordinate changes across your whole project. Think of it as giving the AI a complete task to handle end-to-end, rather than manually guiding each individual file change.
...
2. **Open the Chat Panel** (if it's not already visible, press `Ctrl+Shift+P` to open the **Command Palette** — a search box that appears at the top of the editor — then type "Chat: Focus on Chat View" and press Enter).
3. **Click the mode selector dropdown** at the top of the Chat panel (it will say "Chat" by default) and choose **Agent** from the dropdown.
4. **Type the following prompt** in the chat input and press **Enter**:
...
5. **Watch Agent mode work.** It will automatically:
   - Scan all relevant files in the `src/services` folder
...
6. **Review each proposed change** in the side-by-side diff view — click **Keep** to accept or **Undo** to reject individual file changes.
...
> **What are checkpoints?** Agent mode automatically creates checkpoints (snapshots of your code) before it starts making changes. If you want to revert all the changes made during an Agent session, you can use the checkpoint system to restore your code to its previous state. Access checkpoints through the Chat Panel's checkpoint menu.
...
- **Checkpoints are available** (Agent mode only) — If you used Agent mode, checkpoints are created automatically. You don't need to manually create them. To verify: open the Chat Panel, look for the checkpoint icon or menu, and confirm you can see the restore point from before the Agent session started.

**Congratulations!** You've applied professional-grade clean code refactoring techniques using Code Studio's AI tools. Your codebase is now more readable, more maintainable, and better prepared for new features.
